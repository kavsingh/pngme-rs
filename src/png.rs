...
impl TryFrom<&[u8]> for Png {
	...
	fn try_from(input: &[u8]) -> Result<Self, Self::Error> {
		if input[0..8] != Self::STANDARD_HEADER {
			return Err(Self::Error::from("invalid header"));
		}
		...
		while i < input.len() {
			let chunk = Chunk::try_from(&input[i..])?;
...
impl Png {
	...
	pub fn remove_first_chunk(&mut self, chunk_type: &str) -> crate::Result<Chunk> {
		let match_bytes = chunk_type.as_bytes();
		let mut i = 0;

		while i < self.chunks.len() {
			match self.chunks.get(i) {
				Some(c) if c.chunk_type().bytes() == match_bytes => {
		...
		Err(crate::Error::from("no matching chunk"))
	}
	...
	pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
		...
		self.chunks
			.iter()
			.find(|c| c.chunk_type().bytes() == match_bytes)
	}
